...
function setupEventListeners() {
    ...
    // ⭐ NEW: Event listeners untuk tombol riwayat
...
async function loadAttendanceData() {
    try {
        ...
        const response = await fetch('presensi.json');
        ...
        console.log('🔍 Data structure sample:', data.slice(0, 3));
        
        // Simpan data ke localStorage untuk rekap
        localStorage.setItem('attendanceData', JSON.stringify(data));
        
        // Filter data berdasarkan jurusan, angkatan, dan tanggal
...
function displayData(data) {
    ...
    console.log('🖥️ Displaying data:', data);
    ...
    if (!data || Object.keys(data).length === 0) {
        ...
        if (jurusanLabelSpan) jurusanLabelSpan.textContent = 'Jurusan Terfilter';
        return;
    }
...
function showAllHistoryData() {
    ...
    data.forEach((record, index) => {
        console.log(`${index + 1}. ${record.nama} - ${record.tanggal} ${record.waktu_presensi} - ${record.status}`);
    });
